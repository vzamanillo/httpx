...
// DefaultClientOptions contains the default options
...
func (options *Options) validateOptions() error {
	if options.InputFile != "" && !fileutils.FileExists(options.InputFile) {
		return fmt.Errorf("file %s does not exist", options.InputFile)
	}

	if options.InputRawRequest != "" && !fileutils.FileExists(options.InputRawRequest) {
		return fmt.Errorf("file %s does not exist", options.InputRawRequest)
	}

	var err error

	if options.MatchStatusCode, err = sliceutils.ToIntSlice(options.OutputMatchStatusCode); err != nil {
		return fmt.Errorf("invalid value for match status code option: %s", err)
	}

	if options.MatchContentLength, err = sliceutils.ToIntSlice(options.OutputMatchContentLength); err != nil {
		return fmt.Errorf("invalid value for match content length option: %s", err)
	}

	if options.FilterStatusCode, err = sliceutils.ToIntSlice(options.OutputFilterStatusCode); err != nil {
		return fmt.Errorf("invalid value for filter status code option: %s", err)
	}

	if options.FilterContentLength, err = sliceutils.ToIntSlice(options.OutputFilterContentLength); err != nil {
		return fmt.Errorf("invalid value for filter content length option: %s", err)
	}

	if options.OutputFilterRegex != "" {
		if options.FilterRegex, err = regexp.Compile(options.OutputFilterRegex); err != nil {
			return fmt.Errorf("invalid value for regex filter option: %s", err)
		}
	}

	if options.OutputMatchRegex != "" {
		if options.MatchRegex, err = regexp.Compile(options.OutputMatchRegex); err != nil {
			return fmt.Errorf("invalid value for match regex option: %s", err)
		}
	}
